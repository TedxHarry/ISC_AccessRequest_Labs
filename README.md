...
The course follows Acme Corporation's IAM team through **75 practice labs and three capstone projects**. You will work with the same identities, AD groups, access profiles, roles, and applications as the scenarios become more challenging.
...
| ID | Type | Assignment | Skill and required proof |
|---|---|---|---|
| AR-010 | Build | Make VPN access requestable | Configure the catalog item and capture requester visibility |
| AR-011 | Build | Require review and a business reason | Predict the reviewer; verify submitted information and routing |
| AR-012 | Practice | Complete the first successful request | Link submission, decision, activity, and AD membership |
| AR-013 | Practice | Deny a second user's request | Prove the denial outcome and absence of a new target grant |
| AR-014 | Investigate | VPN is missing from Request Center | Isolate the requestability fault with a non-admin session |
| AR-015 | Check | Deliver Remote Users access without a walkthrough | Build, test, and explain a second end-to-end request |
...
| ID | Type | Assignment | Skill and required proof |
|---|---|---|---|
| AR-016 | Build | Create Finance Reporting access | Bundle two groups and verify both target memberships |
| AR-017 | Build | Present Finance access through its application | Validate the application's requester experience |
| AR-018 | Build | Offer a Finance Analyst role | Trace the requested role to profiles and target memberships |
| AR-019 | Practice | Compare the three requestable object types | Explain ownership, packaging, and assignment differences |
| AR-020 | Investigate | An application exists but offers no usable access | Diagnose application/profile configuration with evidence |
| AR-021 | Check | Onboard HR Services from business requirements | Choose an access model, test it, and save C03 |
...
| ID | Type | Assignment | Skill and required proof |
|---|---|---|---|
| AR-022 | Build | Scope Finance catalog visibility | Test Finance and non-Finance requester sessions |
| AR-023 | Practice | Compare employee, manager, and admin request options | Produce a tested actor/recipient permission matrix |
| AR-024 | Practice | Request for a colleague in another department | Establish whose visibility and whose request authority apply |
| AR-025 | Investigate | A Finance employee cannot find Finance access | Trace the incorrect HR attribute through the identity and segment |
| AR-026 | Practice | Create Sofia's second account and test account selection | Configure baseline account-selection criteria before introducing the second account; prove which account receives requested membership |
| AR-027 | Check | Resolve three visibility and account-selection tickets | Diagnose independently and distinguish separate causes |
...
| ID | Type | Assignment | Skill and required proof |
|---|---|---|---|
| AR-028 | Build | Compare manager, item-owner, and source-owner routing | Record expected versus actual reviewer for each supported setup |
| AR-029 | Build | Add ordered review and a Governance Group | Demonstrate review order and who can complete the group review |
| AR-030 | Investigate | A request has a missing or unsuitable reviewer | Compare missing-manager and inactive-owner variants; establish documented recovery |
| AR-031 | Practice | Test requester/reviewer overlap and reassignment | Verify self-approval handling and an authorized reassignment |
| AR-032 | Investigate | Reminder or escalation behavior seems wrong | Compare effective settings, submission time, and notification evidence |
| AR-033 | Check | Repair an approval process and regression-test it | Include approval, denial, timeout planning, and save C05 |
...
| ID | Type | Assignment | Skill and required proof |
|---|---|---|---|
| AR-034 | Build | Create a production-support request form | Configure business reason, ticket number, and environment |
| AR-035 | Practice | Test required and optional inputs | Record accepted and rejected submissions |
| AR-036 | Practice | Explore documented conditional-field behavior | Test visible/hidden field combinations; record limitations |
| AR-037 | Practice | Trace submitted information to the reviewer | Identify actual display locations and available evidence |
| AR-038 | Investigate | The expected form does not appear | Diagnose item association/configuration with a control request |
| AR-039 | Check | Deliver a usable request form from requirements | Prove validation and explain what the form does not enforce |
...
| ID | Type | Assignment | Skill and required proof |
|---|---|---|---|
| AR-040 | Build | Request temporary production-support access | Test supported date constraints and capture effective dates |
| AR-041 | Practice | Observe future start and expiration | Verify target state before/after events; record the effective access dates |
| AR-042 | Build | Configure and execute access removal requests | Compare supported entitlement/profile/role removal paths |
| AR-043 | Investigate | Removal completed but VPN membership remains | Trace deliberately overlapping grants and the remaining path |
| AR-044 | Practice | Compare requested and automatically assigned access | Explain supported removal and potential reassignment behavior |
| AR-045 | Check | Remove the intended access and preserve unrelated grants | Verify assignment and AD state; save C07 |
...
| ID | Type | Assignment | Skill and required proof |
|---|---|---|---|
| AR-046 | Investigate | Access was approved but is absent | Classify the failing stage before changing configuration |
| AR-047 | Investigate | The requested-for identity has no AD account | Examine supported account-creation prerequisites and actual operation results |
| AR-048 | Investigate | The provisioning account cannot change a lab group | Diagnose a scoped permission failure and validate recovery |
| AR-049 | Investigate | The requested group reference no longer resolves | Compare aggregated entitlement identity with the target object |
| AR-050 | Investigate | AD and ISC show different access state | Distinguish target state, aggregation, and search refresh evidence |
| AR-051 | Check | Resolve a multi-symptom fulfillment incident | Build a timeline, justify root cause, recover, and retest |
...
| ID | Type | Assignment | Skill and required proof |
|---|---|---|---|
| AR-052 | Practice | Authenticate and read the request configuration | Distinguish authentication, authorization, and API-version errors |
| AR-053 | Build | Submit one request and track its outcome | Use current payloads and distinguish acceptance from fulfillment |
| AR-054 | Practice | Investigate a request across related API resources | Map returned identifiers to UI/activity evidence; handle pagination |
| AR-055 | Practice | Test approval, rejection, and cancellation operations | Use permitted actors and explain operation/state restrictions |
| AR-056 | Investigate | A client loses a response and proposes a retry | Check existing state before resubmission; use fixtures for throttling/error cases |
| AR-057 | Check | Produce a repeatable request investigation report | Build a parameterized read-only script and save C09 |
...
| ID | Type | Assignment | Skill and required proof |
|---|---|---|---|
| AR-058 | Build | Attach a simple approval workflow | Prove that the correct access item invokes the enabled workflow |
| AR-059 | Build | Compare serial and parallel multi-step review | Demonstrate timing and final decisions using separate requests |
| AR-060 | Practice | Test a quorum approval policy | Predict and test sufficient approvals, denial outcomes, and early completion |
| AR-061 | Build | Route a request using a documented identity attribute | Verify both branches and unavailable-data behavior |
| AR-062 | Investigate | The workflow never starts or follows the wrong branch | Trace association, enablement, payload, and execution evidence |
| AR-063 | Check | Deliver conditional approval with decision notification | Validate approval/denial and compare new versus in-flight requests |
...
| ID | Type | Assignment | Skill and required proof |
|---|---|---|---|
| AR-064 | Practice | Compare the two Submitted mechanisms | Produce separate payload, configuration, and timing records |
| AR-065 | Build | Return a preliminary decision from the subscriber | Verify successful and denied control requests |
| AR-066 | Build | Add a reviewer through Dynamic Approval | Prove the additional review and valid identity/group resolution |
| AR-067 | Practice | Observe the request decision event | Match subscriber evidence to the request's actual decision |
| AR-068 | Investigate | An external subscriber disrupts requests | Diagnose no response/invalid response variants; restore the baseline |
| AR-069 | Check | Recover an integration and prove normal requests work | Validate affected and fresh requests; record remaining side effects |
...
| ID | Type | Assignment | Skill and required proof |
|---|---|---|---|
| AR-070 | Practice | Triage a queue of aged requests | Classify approval, provisioning, timing, and evidence gaps |
| AR-071 | Practice | Decide whether a request should be canceled or closed | Apply documented eligibility, capture audits, and verify target state separately |
| AR-072 | Build | Change an owner and approval configuration | Record impact, apply the change, and regression-test fresh requests |
| AR-073 | Investigate | Diagnose a partial or uncertain fulfillment report | Examine item/account evidence and propose justified recovery |
| AR-074 | Practice | Prepare an operational handover and support package | Write a reproducible incident report with sanitized evidence |
| AR-075 | Check | Run a simulated IAM support shift | Prioritize mixed tickets, resolve supported cases, and escalate the rest |
