...
Complete [AR-040](../AR-040/README.md), including removal. Use Acme Admin, Henry, Ava, Evelyn and the AD workstation. Keep the required end date, seven-day maximum and any attached form. You need a window when you can complete both approvals before the start and observe AD before, during and after access. Keep your [journal](EVIDENCE.md) open.

## Follow the steps

### 1. Submit a future window

1. Confirm Henry has no Production Support assignment or pending request and direct Support membership is False. Baseline must remain True. Use [the membership procedure](../../M02-CHECKS.md#inspect-direct-ad-membership).
...
3. As Henry, select Production Support in **Request Center > Access Items > Access Profiles**. Complete its attached form with ticket `CHG-LAB-041-A`, Production, description `Observe scheduled lab support access`, rollback `Verify scheduled removal`, and comments `AR-041 scheduled window`. Set the standard start/end to your recorded window.
...
2. Before S arrives, open Henry's **My Access > Access Profiles**. On Production Support, select **Edit** for its start date. Change the start to **S + 1 hour** and keep the original end **S + 4 hours**. Enter comments `AR-041 later start and shorter duration`, then **Save**. The resulting duration is three hours.
3. As administrator, inspect the new date-change request's **Process/Assignees/Details**. This change uses the configured removal approval because the start moved later and the total duration shortened. Ava is the removal reviewer; Security should not be a second removal stage.
...
3. As administrator, verify this route uses grant review even though the duration shortened. As Ava, inspect the old/new dates and approve; as Evelyn, approve the Security stage. Confirm the effective assignment now has the latest approved dates. Save `AR-041-04.png`.
