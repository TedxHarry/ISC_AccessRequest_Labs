...
3. Combine the pages in your private evidence notes. Check for repeated status `id` values across pages and compare the combined count with X-Total-Count when provided. Keep the original pages if counts disagree or a row repeats; rerun from offset 0 after requests settle rather than silently dropping rows. Count item-status rows, not unique parent requests. Record page offsets/counts and locate the AR-053 Grant and its separate removal. Use created time, request type and requester comment; the same profile name appears in more than one request.
...
2. Select Acme API Priya. Send **GET `{{apiBase}}/access-request-approvals/v1/pending`** with Params `owner-id=me`, `limit=50`, `offset=0`, and `filters=requestedFor.id eq "{{recipientId}}"`. Enter the filter in Params so Postman encodes it. Read further pages if necessary. If the new approval is not visible yet, wait about 30 seconds and repeat the queue/status GETs. Do not send a second grant.
