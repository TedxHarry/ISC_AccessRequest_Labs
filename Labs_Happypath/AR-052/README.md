...
1. Follow [Prepare a private client environment](../../API-WORKBENCH.md#prepare-a-private-client-environment) once as Acme Admin, once as Taylor and once as Priya. Create the named environments **Acme API Admin**, **Acme API Taylor** and **Acme API Priya** with each person's own PAT and local vault values.
...
3. Send each environment's **POST `{{apiBase}}/oauth/token`** exchange using its own Client ID/Secret. Check HTTP 200, store its bearer token locally and record expiry. Do not copy the administrator token into Taylor's or Priya's environment.
