...
On a repeat run, reuse the matching caller environment and named PAT if its credentials are still valid and available. Renew an expired bearer token through the token exchange below; create another PAT only when you need new credentials.

1. On a repeat run, reuse the matching caller environment and named PAT if its credentials are still available and valid. Renew an expired bearer token with Steps 5–6; create a PAT only if you do not have a usable one. For a new PAT, sign in as the person named by the lab. Verify the username in the user menu, then open **Preferences > Personal Access Tokens > New Token**. Enter a description such as `Acme Module 9 Taylor`, then **Create Token**. Keep the dialog open until its Client ID and Client Secret are stored in Step 4. Record the owner and token description, never the secret. Do this in the owner's session, not in an administrator session on their behalf.
...
1. Verify the grant operation completed and both native memberships are True. [Refresh imported AD data](M02-CHECKS.md#refresh-imported-ad-data) and confirm Taylor has the Remote Worker assignment. Resolve any remaining grant/assignment discrepancy before attempting removal. As Taylor in ISC, open **My Access > Access Profiles > AP-Remote-Worker > Revoke Access Profile**. Enter the current lab number and `control complete`, then **Submit**.
...
4. Recheck Taylor's request history and Priya's pending approvals. Resolve other diagnostic requests before another grant. Keep baseline and the disposable group absent for Taylor; retain the account and the complete private HR file, including Taylor and any later-course records. The first pass has 25 rows; do not replace a larger current file with that older population.
