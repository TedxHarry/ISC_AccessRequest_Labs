...
Use the capture list at the end of the [lab](README.md). Name the files `AR-011-01.png`, `AR-011-02.png` and so on in that order. Record a separate filename for each test variant. Exclude credentials and callback secrets.
...
|---|---|---|
| Recipient / reviewer | Lucas / Priya | |
| Reviewer category | Primary Owner | |
| Request state | One pending Grant request from AR-011 | |
| Reason | AR-011: Remote access for the Finance lab | |
| Native VPN | Absent while pending | |
