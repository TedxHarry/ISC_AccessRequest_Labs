...
1. As administrator, open **Admin > Global > Forms > FORM-Acme-Production-Support** and select **Edit in Builder**. Verify Change ticket (`changeTicket`), Environment (`environment`) and Work description (`workDescription`) are required. Implementation notes (`implementationNotes`) and Rollback plan (`rollbackPlan`) remain optional.
...
Record Henry’s standard account DN, objectGUID and the domain controller used by the native check in your journal. Use that same account and controller for the grant and removal checks below.
...
2. Clear only Change ticket and attempt to save/continue. Record the error, then restore the ticket. Clear only Environment, record its error and restore Test. Clear only Work description, record its error and restore the text. These attempts should not create submitted request IDs. If a missing answer does not block progress, remove the unsent item, correct its saved required setting in the builder and retry with a fresh form. If you accidentally submitted, have Ava deny that ID and verify native Support membership False before retrying.
...
4. As Ava, open **Approvals > Access Requests > Requested** and open Henry's matching Production Support **Grant** details. Compare each answer with your journal. Select **Deny**, enter `AR-039 Test control complete`, and confirm. As administrator, verify **Denied** under **Admin > Dashboard > Approval Management > Access Requests** using that request ID. Check Henry's direct `GG-PROD-SUPPORT` membership is **False** using [the AD membership procedure](../../M02-CHECKS.md#inspect-direct-ad-membership). An error is not a False result. Save `AR-039-02.png` for validation and the resulting diagnostic denial.
...
6. Follow [Account Activity](../../LAB-DESK.md#find-the-account-activity) for Henry's support-group addition. Record the activity ID, source and operation. Wait for successful completion before running the native membership check. If it fails or stays pending, investigate that activity before retrying the request. Expected results: Support must become True, VPN and Remote Users stay False, baseline stays True. Save `AR-039-04.png`.
...
1. After the grant completes, [refresh imported AD data](../../M02-CHECKS.md#refresh-imported-ad-data) and reopen **Admin > Identity Management > Identities > Henry > Access > Access Profiles** as administrator. Verify the granted assignment is visible. Select `AP-Production-Support`, open **Details**, then **Revoke Access Profile**. Enter the lab number and `test complete`, then confirm **Revoke**. Record the removal request ID.
...
3. Follow [Account Activity](../../LAB-DESK.md#find-the-account-activity) for the removal. Record its activity ID and wait for successful completion. Run [the direct AD membership check](../../M02-CHECKS.md#inspect-direct-ad-membership) for `acme.e018`: `GG-PROD-SUPPORT` must be **False** and `GG-ACME-BASELINE` **True**.
4. [Refresh imported AD data](../../M02-CHECKS.md#refresh-imported-ad-data). Reopen Henry's identity and verify the Production Support assignment and group are removed. Compare his DN and objectGUID with the before-state record. Preserve his AD account and baseline role. Resolve a failed removal before another grant test.

5. Recheck VPN and Remote Users remain False. Save `AR-039-05.png` showing completed removal and final memberships.

**Check:** Removal completed, Henry’s standard account and baseline remain, and all three test business groups are absent.

### 5. Save C06

1. Create a private folder named `C06` alongside your earlier checkpoint folders. Complete the [journal](EVIDENCE.md) with the validation cases, request IDs, decisions, activity IDs and final native results. Label an unexecuted case **Not run**; a written expected result is not a tenant observation.
...
4. Save your completed journal and captures in **C06** for AR-040. Keep tenant identifiers and private records outside the public repository. If any check failed, name the failed case and resolve it before calling C06 complete. If native forms are unavailable, mark Module 6 unexecuted rather than treating a standalone form preview as a pass.
...
### Close a misleading form ticket

Use the journal to answer: “The form says rollback in two hours, so why was access not removed automatically?” Identify the exact field holding that text and compare it with the standard access end date. No end date was set in this module. A free-text promise does not schedule removal; AR-040 starts the date-policy exercises. Confirm the profile was actually revoked here and cite the removal activity and native group result.

Also explain why a nonempty ticket is not proof of external approval, why a hidden value is not proof of erasure, and why a form answer does not automatically become an AD attribute. Use the matching earlier lab evidence for each answer.

## Finish

Run the direct membership check for Lucas (`acme.e012`) and verify his retained `GG-VPN-USERS` membership is **True**. Record the result without changing that assignment.
