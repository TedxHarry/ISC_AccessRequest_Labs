...
3. Open **Admin > Access Model > Access Profiles > AP-Remote-Worker > Access Requests**. Record its current required-form checkbox, selected form, grant/removal reviewers, comments and date settings. Its grant reviewer should be Primary Owner, Priya. Save the original configuration in the journal before changing it. On a repeat run, use the original record from your first attempt so the temporary attachment does not become your restore target.
4. Enable **Require Access Request Form**, select `FORM-Acme-Production-Support`, and **Save**. If **Apply Changes** is offered, select it and wait for completion. Reopen to verify. Leave other settings unchanged. Confirm Production Support still uses the same form and Manager then Security review. Save `AR-037-01.png` showing the two associations.
...
3. For Remote Worker, first set Environment to `Production` so Rollback plan is visible. Clear its copied rollback text and Implementation notes. Then set ticket `CHG-LAB-037-B`, Environment `Test`, Work description `Compare the remote access review without granting it`, and comments `AR-037 Remote Worker control`. Select **Save**. In **Review Request > Edit Request Details**, briefly select Production to confirm Rollback plan is empty, then return to Test and save again.
4. Select **Review Request**. Open **Edit Request Details** separately for each profile. Verify ticket A belongs to Production Support and ticket B to Remote Worker. If an edit affected both, correct them and recheck both item summaries before submission. If you cannot retain separate answers, remove both unsent items and record the behavior before retrying; do not submit the comparison with mixed answers. Capture `AR-037-03.png` showing the final association of each item with its answers.
...
3. In Acme Admin, find the Production Support ID under **Admin > Dashboard > Approval Management > Access Requests**. Open the access name and confirm **Process/Assignees** now show GOV-Security-Review. Before Evelyn acts, run the direct AD check and record `GG-PROD-SUPPORT` as **False**. If it is already True, stop and investigate the grant’s origin before approving anything else.
...
5. Inspect [Account Activity](../../LAB-DESK.md#find-the-account-activity) for the resulting Production Support operation. Wait for the operation to complete successfully, then run direct membership checks: `GG-PROD-SUPPORT` must be **True**, `GG-VPN-USERS` and `GG-REMOTE-USERS` **False**, and `GG-ACME-BASELINE` **True**. If provisioning fails or remains pending, inspect that activity before submitting another request. Save `AR-037-04.png` showing both item decisions and target results.
...
1. Reopen Remote Worker's **Access Requests** settings. Restore the form selection recorded in Step 1 while the selector is enabled, then restore the checkbox state. If no form was originally selected, clear the temporary selection before switching the checkbox off. Save, apply changes if offered, then reopen to confirm. Keep the Production Support association.
2. After the grant completes, [refresh imported AD data](../../M02-CHECKS.md#refresh-imported-ad-data) and reopen **Admin > Identity Management > Identities > Henry > Access > Access Profiles** as administrator. Verify the granted assignment is visible. Select `AP-Production-Support`, open **Details**, then **Revoke Access Profile**. Enter the lab number and `test complete`, then confirm **Revoke**. Record the removal request ID.
...
4. Follow [Account Activity](../../LAB-DESK.md#find-the-account-activity) for the removal. Record its activity ID and wait for successful completion. Run [the direct AD membership check](../../M02-CHECKS.md#inspect-direct-ad-membership) for `acme.e018`: `GG-PROD-SUPPORT` must be **False** and `GG-ACME-BASELINE` **True**.
...
**Check:** Henry’s support grant is removed, the baseline account remains, and Remote Worker has its original form configuration.
...
Use the request records for your two items to investigate this report: “I denied one item, but the other still provisioned.” Inspect each item's Process and decision. Explain the difference between a shared basket, a shared form definition and independent item approvals. If you need to cancel a pending item, inspect its request ID first: items sharing an ID are canceled together. Do not cancel a completed grant as a substitute for removal.
