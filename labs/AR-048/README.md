...
If an earlier attempt left the temporary deny in place, restore it using Section 5.1 and your original ACL record before repeating the working control. Never record that temporary deny as the new baseline. Resolve earlier retries and remove any surviving test grant before beginning. Record Taylor’s account DN/GUID and verification controller.

1. In **Active Directory Users and Computers**, open **AcmeLab > Groups**. First look for the group by its saved GUID/current name if repeating this lab. If it already exists, reuse it and skip creation. Otherwise right-click Groups, choose **New > Group**, enter `GG-ACME-FAULT-048`, select **Global** and **Security**, then **OK**. Use description `Disposable membership-permission lab group`. On a new group, keep it empty and unnested. On an existing group, inspect membership and resolve the earlier grant through ISC rather than clearing members manually.
...
3. [Refresh imported AD data](../../M02-CHECKS.md#refresh-imported-ad-data), then as Taylor reopen **My Access > Entitlements > disposable group > Assignment**, verify his account, select **Revoke Assignment**, enter `AR-048 control cleanup`, and **Submit Request**. As Priya, inspect and approve the Remove request. Verify membership False and no pending request.
...
3. Set **Type: Deny** and **Applies to: This object only**. Show advanced permissions if necessary and clear any preselected permissions. Select only **Write members** (the group's `member` property); do not select Full control, Write all properties, Read members or OU-wide permissions. Keep inheritance unchanged.
...
If you pause while the fault is active, restore the added deny using Section 5.1 before leaving. Recheck the original request afterward: restoring permission may allow its queued retry to grant access, which still needs cleanup.
