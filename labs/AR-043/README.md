...
4. Open **Admin > Global > System Settings > Feature Settings > Access Requests**. Record the original **Enable Requests on Behalf of others** state and mode. On a repeat run, use the original journal entry rather than recording the temporary mode as the restore target. Temporarily enable it with **By Everyone for Anyone**, save and reopen. Keep the Finance segment enabled. Lucas will request the Finance item for Alexander; do not change Alexander's department to make it visible.
...
7. Restore the original request-on-behalf state/mode from Step 1. While enabled, set the saved mode first; then restore the original enabled/disabled state. Save and reopen it. Do this now; the remaining steps do not need on-behalf permission.
...
Before removal, write down whether you expect VPN to remain and why. Keep that prediction for the engineering comparison.

[Refresh imported AD data](../../M02-CHECKS.md#refresh-imported-ad-data) and reopen Alexander’s access before selecting the profile to revoke.
...
3. Refresh imported AD data after the re-grant, then as Alexander reopen **My Access > Access Profiles > AP-Remote-Worker**, select **Revoke Access Profile**, enter `AR-043 cleanup complete`, and **Submit**. As Priya, inspect and approve the removal. Follow its activity and verify VPN and Remote Users both False.
4. Refresh imported AD data. Verify all three business groups False, baseline True and no pending test request. Save `AR-043-03.png`. Reconfirm the original request-on-behalf configuration is restored. Compare Alexander’s final DN/GUID with the starting values and run the direct check for Lucas’s retained VPN=True.
...
Compare your prediction from Step 3 with the actual removal operation and native result. If VPN remains, inspect business role/lifecycle assignments and other provisioning activity before calling the service inconsistent. Do not confuse the baseline role, which requires only GG-ACME-BASELINE, with a role requiring VPN. Keep the exact assignment origins in the ticket explanation.
