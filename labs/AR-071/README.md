...
5. If the request is still stuck in Pending, open **Search > Account Activity** and locate it with the [exact activity lookup procedure](../../LAB-DESK.md#find-the-account-activity), using the request recipient, exact source and submission time. Copy the **Tracking Number** from the matching row; keep it separate from the request ID and activity ID. Follow the explicit close example in the API workbench. A historical terminal failure is not eligible merely because it once failed. If no eligible stuck request exists, complete the payload/eligibility review without sending it and mark live closure untested.
