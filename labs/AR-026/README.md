...
2. In AD Users and Computers, right-click **AcmeLab > AdminAccounts > New > User**. Enter Sofia / Martin for first and last name, `Acme Lab - Sofia Martin Admin` for Full name, and `acme.e009.admin` for User logon name and the pre-Windows 2000 username. Select the suffix recorded in [AR-005](../AR-005/README.md#find-and-record-your-upn-suffix), then Next. Enter a private test password meeting domain requirements and finish. Open Properties > General and verify Display name is `Acme Lab - Sofia Martin Admin`. Do not give it domain-administrator privileges.
3. Enable **View > Advanced Features**, open the new user directly from the AdminAccounts OU, then **Properties > Attribute Editor** and set `employeeID` to `E009`. Use the [native AD lookup](../../LAB-VALUES.md#read-an-ad-objectguid-and-account-attributes) with username `acme.e009.admin` to record its DN and objectGUID. This permits AR-004's employeeID correlation to the same Sofia identity.
